...
/**
 * Summary: Library taken from internet to dynamically parse json
 */
...
object JSON_PARSER {
  ...
  def makeJSON(a: Any): String = a match {
    ...
    case l: List[_] => l.map(makeJSON).mkString("[", ",", "]")
    case l: java.util.List[_] => l.map(makeJSON).mkString("[", ",", "]")
    case s: String => "\"" + s + "\""
...
case object JSONException extends Exception
...
class ScalaJSON(o: java.lang.Object) extends Seq[ScalaJSON] with Dynamic {
  ...
  def keySet: util.Set[String] = o match {
    case m: JSONObject => m.keySet()
    case _ => throw JSONException
  }
